...
Puis dans un deuxième temps, nous verrons une vision BI de nos données à l'aide de l'analyse non supervisée
...
list.of.packages= c("data.table", "naniar", "ggplot2", "dplyr", "tidyr", "rworldmap"
                    , "corrplot", "rstudioapi", "VIM", "FactoMineR", "factoextra")
...
data <- na.omit(data) # Remove potential NA (au cas ou)
dim(data)
colnames(data)
```
...
```{r}
#On peut exporter les données avec les différents clusters pour Tableau
data_cluster_geo= data[, c("geographical_cluster", "city")]

data_grouped= aggregate(data_cluster_geo$geographical_cluster, by= list(data_cluster_geo$city)
                        , mean
                        , na.rm= TRUE)
write.csv(data_grouped
         , file = "data/data_cluster.csv")
rm(data_cluster_geo)
rm(data_grouped)
